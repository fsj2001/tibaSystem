	...
	function setTree(data,that)
	{
		...
		$.each(data,function(index,value){
			...
			if(value.children.length == 0){
				li = $('<li><span onclick="f1('+value.value+')">'+value.name+'<i data-val="'+value.value+'" class="fa fa-check" style="display:none;"></i></span></li>');
			}
			else{
				li = $('<li><span>'+value.name+'<i data-val="'+value.value+'" class="fa fa-check" style="display:none;"></i></span></li>');
			}
			ul.append(li);
		    if(value.children.length > 0)
		    {
...
function f1(parameter) {
	window.open("../test.html?id="+parameter);
}